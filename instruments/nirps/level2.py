...
from instruments.nirps.utils import convert_S2D_BLAZE, convert_BLAZE, convert_DRIFT, get_files_names, create_PRIMARY, validate_fits_file, convert_RAW
...
class NIRPSRV2(RV2):
    ...
    This class extends the `RV2` base class to handle the reading of NIRPS (Near Infra Red Planet
    Searcher) Level 1 and Level 2 files, combining information from both sources to produce a 
    standardized EPRV output. It processes various FITS extensions and organizes flux, wavelength, 
    variance, and metadata into a structured Python object.
