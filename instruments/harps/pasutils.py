'''
RVData/instruments/harps/utils.py

UNIGE-ESO - EPRV
Author: Loris JACQUES
Created: Tue Jan 07 2025
Last Modified: Tue Jan 14 2025
Version: 1.0.0
Description: 
'''
...
def get_files_names(full_path:str) -> dict:
    ...
    # On Windows, ":" in file names is replaced with "_" as ":" is not allowed in Windows file systems.
...
def convert_S2D_BLAZE(RV2: RV2, file_path: str, trace_ind_start: int, slice_nb: int) -> None:
    ...
    This function reads the specified FITS file, extracts relevant information, and stores 
    the processed data in the RV2 object. The function ensures that certain header values 
    are only set once during the first iteration. It also applies a Doppler shift correction 
    when necessary and organizes the extracted data into new FITS extensions.
        ...
        for field in config.extnames.keys():
            for slice in range(1, slice_nb+1):
                ...
                if(len(single_cam_values) != config.NUMORDER):
                    single_cam_values = add_nan_row(single_cam_values, config.empty_raw_order)
...
def convert_BLAZE(RV2: RV2, file_path: str, trace_ind_start: int, slice_nb: int) -> None:
    """
    This function processes a FITS file and converts its data into multiple 'BLAZE' extensions, 
