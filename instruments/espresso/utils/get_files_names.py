    ...
    if not os.path.isfile(drift_file_B):
        with fits.open(full_path) as hdu_raw:
            ...
            if dpr_type.split(",")[1] == 'SKY':
                ...
                drift_file_B = None
                # raise ValueError(
                #     f"Error: File DRIFT not found. Conversion not possible."
                # )
...
def adjust_repo_path(repo_path: str, blaze_filename: str) -> str:
    ...
    If the file's timestamp is before noon, it should be placed in the previous
    day's directory.
    ...
    try:
        ...
        timestamp_str = filename_parts.split("r.ESPRE.")[1]
        ...
        if ':' in timestamp_str:
            ...
            file_datetime = datetime.strptime(
                timestamp_str, "%Y-%m-%dT%H_%M_%S.%f"
            )
        ...
        print(
            f"Error processing repo path for BLAZE file '{blaze_filename}':{e}"
        )
