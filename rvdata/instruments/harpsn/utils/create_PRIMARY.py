...
the header_map.csv file to match TNG keywords with those of the new format.
...
def create_PRIMARY(
                ...
                if (header_map['from'].iloc[index] == 'S2D_BLAZE_A'):
                    l2_hdu.header[values.iloc[0]] = (
                        RV2.headers['INSTRUMENT_HEADER'][
                            header_map['TNG_keyword'].iloc[index]
                        ],
                        header_map['Description'].iloc[index]
                    )
                    ...
                    l2_hdu.header[values.iloc[0]] = (
                        getattr(
                            config, header_map['TNG_keyword'].iloc[index], None
                        ),
                        header_map['Description'].iloc[index]
                    )
    ...
    for keyword in add_keyword_cat:
        catalog_data[keyword] = RV2.headers['INSTRUMENT_HEADER'][
            header_map[header_map['Keyword'] == keyword]['TNG_keyword'].iloc[0]
        ]
    ...
    with fits.open(names["raw_file"]) as hdu_raw:
        dpr_type = (
            hdu_raw['PRIMARY'].header['HIERARCH TNG DPR TYPE'].split(",")
        )
    ...
    for i in range(1, int(l2_hdu.header['NUMORDER'])+1):
        l2_hdu.header[f'EXSNR{str(i)}'] = (
            RV2.headers['INSTRUMENT_HEADER'][
                f"HIERARCH TNG QC ORDER{str(i)} SNR"
            ],
...
def get_gaia_data(gaia_name: str) -> float:
    ...
    Args:
        gaia_name (str): The Gaia source identifier in the format "DR3
            <source_id>".

    Returns:
        color_br (float): The computed color index (BP - RP) for the specified
            Gaia source.
    """
...
def parse_geo_coord(coord_str: str) -> float:
    """
    Convert a coordinate string (latitude or longitude) in DMS format to
        decimal degrees.

    Args:
        coord_str (str): Coordinate string in the format 'DD MM SS.s D', where
            D is the direction (N, S, E, W).
    ...
    degrees = int(parts[0])
    minutes = int(parts[1])
    seconds = float(parts[2].rstrip("WENS"))
...
def deg_to_sexagesimal(value_deg: float, is_ra: bool = False) -> str:
    """
    Converts a value in degrees to sexagesimal format (HH:MM:SS.sss
    or ±DD:MM:SS.sss).
...
def get_moon_sun_info(
    ...
    Returns:
        list: A list containing the following values:
            ...
            moon_el (float): The elevation of the Moon at the observation site
                (in degrees).
...
def get_moon_velocity_in_target_direction(
    ...
    # Get the Moon's barycentric position & velocity (in AU and AU/day) using
    # JPL ephemeris
...
def get_azimuth_target(
        target_ra: float, target_dec: float,
        obs_lat: float, obs_lon: float,
        obs_alt: float, obs_time: str
) -> float:
    """
    Calculates the azimuth of a celestial target from a given observation
    location and time.
    ...
    Args:
        ...
        obs_time (str): Observation time in ISO format
            (e.g., "2024-02-19T12:00:00").

    Returns:
        target_azimuth (float): The azimuth of the target in degrees, rounded
