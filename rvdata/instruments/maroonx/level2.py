...
class MAROONXRV2(RV2):
    ...
    Level 1 files and converts them into a standardized EPRV format. Each
    extension from the FITS file is read, and relevant data, including flux,
    wavelength, variance, and metadata, are stored as attributes of the
    resulting Python object.

    Methods
    -------
    _read(self, file: str) -> None:
        Reads the input HDF5 file, extracts specific extensions related to
        the science data for different channels and fibers, and stores them
    ...
    extensions : dict
        ...
        `C1_SKY1`, `C2_CAL1`) where the keys are the extension names and the
        values are `SpectrumCollection` objects for each respective dataset.

    header : dict
        A dictionary containing metadata headers from the FITS file, with
    ...
    def standardizeMXHeader(self, prime, file, obstype):
        ...
        name = file
        ra_deg = prime['MAROONX TELESCOPE TARGETRA']  # Right Ascension (deg)
        dec_deg = prime['MAROONX TELESCOPE TARGETDEC']  # Declination (deg)
    ...
    def create_wavehead(self, spec_blue, spec_red, fiber):
        """
        Construct WCS-compliant wavelength header for a given fiber for both
        blue and red channel combined.
        """
        wave_meta = {}
    ...
    def make_orderTable(self, spec_blue, spec_red):
        """
        Generate a combined echelle order table from blue and red channel
        spectra with the starting/ending wavelengths for each order.
        """
        ...
        for orders, wavelengths in [
            (ordR, spec_red['wavelengths'][3]),
            (ordB, spec_blue['wavelengths'][3])
        ]:
            for o in orders:
                wave = wavelengths[o]
    ...
    def _read(self, file: str) -> None:
        ...
        self.set_header("PRIMARY", self.standardizeMXHeader(
            self.header_blue, file, obstype))
        ...
        ext_table["description"].append(
            "Header inherited from native instrument file for Blue camera")
        self.create_extension(
            "INSTRUMENT_HEADER_RED",
            "ImageHDU",
            header=self.clean_key(self.header_red))
        ext_table["extension_name"].append("INSTRUMENT_HEADER_RED")
        ext_table["description"].append(
            "Header inherited from native instrument file for Red camera")

        # Create order table
        order_table_data = self.make_orderTable(self.spec_blue, self.spec_red)
        self.create_extension(
            "ORDER_TABLE", "BinTableHDU", data=order_table_data)
        ...
        for fiber in fiber_range:
            ...
            if fiber == 1:
                ...
                blue_flux = (
                    self.spec_blue['box_extraction'][fiber][:]
                    if fiber == 5 else self.spec_blue[flux_key][fiber][:]
                )
                red_flux = (
                    self.spec_red['box_extraction'][fiber][:]
                    if fiber == 5 else self.spec_red[flux_key][fiber][:]
                )
        ...
        for fiber in fiber_range:
            ...
            for key in ['FLUX', 'WAVE', 'VAR', 'BLAZE']:
                blue[key] = self.pad_array(
                    blue[key], (blue[key].shape[0], max_cols))
                red[key] = self.pad_array(
                    red[key], (red[key].shape[0], max_cols))

            combined = {key: np.concatenate(
                (blue[key], red[key]), axis=0) for key in blue.keys()}

            for key, dtype in zip(
                    ['FLUX', 'WAVE', 'VAR', 'BLAZE'],
                    [np.float32, np.float64, np.float32, np.float32]):
                if fiber == 1:
                    self.set_data(out_prefix + key, combined[key])
                else:
                    if key == 'WAVE':
                        wave_md = self.create_wavehead(
                            self.spec_blue, self.spec_red, fiber)
        ...
        ext_table["description"].append(
            "Barycentric correction velocity in km/s")

        berv_z = berv_kms/3e5  # approx
        ext_table["extension_name"].append("BARYCORR_Z")
        ext_table["description"].append(
            "Barycentric correction velocity in redshift (z)")
        ...
        ext_table["description"].append(
            "Instrument drift velocity measured in blue and red camera "
            "in km/s"
        )
        # OPTIONAL
        ...
        self.create_extension(
            "EXT_DESCRIPT", "BinTableHDU", data=pd.DataFrame(ext_table))
